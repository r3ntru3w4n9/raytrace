    ...
    public static Scene scenes() {
        ...
        for (int i = -11; i < 11; ++i) {
            for (int j = -11; j < 11; ++j) {
                ...
                var center = new Vector(iF + .9 * random.nextDouble(),
                                        .2,
                                        jF + .9 * random.nextDouble());

                list.add(new Sphere(center, .2, randomMaterial()));
            }
        }

        list.add(new Sphere(new Vector(0., -1000., 0.), 1000.,
                            new Matte(Vector.uniform(.9))));

        list.add(new Sphere(Vector.j(), 1.,
                            new Glass(Vector.uniform(1.), 0., 1.5)));
        list.add(new Sphere(new Vector(-4., 1., 0.), 1.,
                            new Matte(new Vector(.4, .2, .1))));
        list.add(new Sphere(new Vector(4., 1., 0.), 1.,
                            new Metal(new Vector(.7, .6, .5), 0.)));
